...
export default {
    ...
    methods: {
        ...
        async verifyOTP() {
            try {
                const response = await axios.get(`http://localhost:8080/api/v1/auth/verify/${this.verifData.phone}/${this.verifData.otp}`);
                ...
                if (response.data.verificationStatus == true) {
                    ...
                    this.message = "Code didn't matched";
                }
            } catch (error) {
        ...
        async resendOtp() {
            const response = await axios.get(`http://localhost:8080/api/v1/auth/send/otp/${this.verifData.phone}`);
            this.message = 'New code has been sent.';
        }
    }
};
</script>
